...
// The string return value is "why"; is not currently used; and is always "app connector transit IP".
...
//
// "some modes of operation": browsers are starting to complain loudly if non-local pages include
...
func (c *client) linkLocalAllow(p *packet.Parsed) (bool, string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.isKnownTransitIP(p.Dst.Addr()), linkLocalAllowReason
}
...
func (c *connector) ConnectorRealIPForTransitIPConnection(srcIP netip.Addr, transitIP netip.Addr) (netip.Addr, error) {
	...
	if !c.config.transitIPSet.Contains(transitIP) {
		fmt.Println("transitIPSet", c.config.transitIPSet, "doesn't contain", transitIP)
		return netip.Addr{}, nil
	}
	return netip.Addr{}, ErrUnmappedSrcAndTransitIP
}

const linkLocalAllowReason = "app connector transit IP"
...
// but in some modes of operation conn25 uses link-local addresses for transit IPs.
// Let the filter know if this is one of our addresses and should be allowed.
func (c *connector) linkLocalAllow(p *packet.Parsed) (bool, string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	_, ok := c.lookupBySrcIPAndTransitIP(p.Src.Addr(), p.Dst.Addr())
	return ok, linkLocalAllowReason
}
