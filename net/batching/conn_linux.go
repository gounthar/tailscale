...
var (
	// disableIOV forces user-space coalescing during batched writes.
	disableIOV = envknob.RegisterBool("TS_BATCHING_DISABLE_IOV")
)
...
//
// This version copies coalesced messages into a single write buffer, which is
// suboptimal if target capacity is below 'maxPayloadLen'.
//
...
// where possible while maintaining datagram order.
//
// This version relies on net/x support for sendmmsg(2),
// deferring coalescing until the kernel.
//
